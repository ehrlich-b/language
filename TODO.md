...
- [x] `std/os.lang` gitignored — generated at build time (no wrong version committed)
- [ ] **TODO**: Proper platform conditional compilation (see Backlog)
...
### Language Features
- [ ] **Platform conditional compilation** - Select code based on target OS/arch
  - **Current workaround**: `std/os.lang` is gitignored, generated by Makefile/test scripts
  - **Options**:
    - Go-style: filename suffixes (`file_linux.lang`, `file_darwin.lang`) auto-selected by compiler
    - Rust-style: `#[cfg(os = "linux")]` attributes
    - C-style: `#ifdef LANGOS_LINUX` preprocessor
    - Zig-style: `@import("builtin").os` comptime check
  - Go-style filename convention is simplest: compiler checks LANGOS, picks `foo_linux.lang` over `foo.lang`
